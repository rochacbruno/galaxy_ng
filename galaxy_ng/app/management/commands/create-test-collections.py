...
from galaxy_ng.tests.unit.constants import TEST_COLLECTION_CONFIGS
...
class Command(BaseCommand):
    ...
    def build_collection(self, template, config):
        ...
        return generator.build_collection(template, config=config)
    ...
    def strategy_faux(self, *args, **options):
        ...
        for _ in range(options["ns"]):
            namespace_name = faux.gen_string(
                "alpha",
                8,
                validator=lambda v: v not in _configs,
                tries=100,
                default=faux.gen_string("alpha", 8),
            ).lower()
            create_inbound_repo(namespace_name)
            np = self.create_namespace(namespace_name)
            _collections = []
            for i in range(options["cols"]):
                config = {
                    "name": faux.gen_string(
                        "alpha",
                        10,
                        validator=lambda v: v not in _collections,
                        tries=100,
                        default=faux.gen_string("alpha", 10),
                    ).lower(),
                    "description": faux.gen_iplum(words=8),
                    "namespace": np.name,
